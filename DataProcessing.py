...
class DataProcessing(object):
    ...
    def process_time(self):
        ...
        for i in range(1,28,1):
            ...
            df['measureTime']=temp
            print(temp)
            df.to_csv("Data_Pro/"+str(i)+".csv",index=None)
            # df['measureTime']=df['measureTime'].replace(df["measureTime"].values,df['measureTime'].str[0:19])
    ...
    def  merge_csv_by_MeasureTime(self,merge_file,on=['measureTime'],how="outer",from_csv=[]):
        ...
        :return:
        """
        pd.read_csv("Data_Pro/1.csv").to_csv("Data_Pro/"+merge_file)
        for i in range(2,len(from_csv),1):
            df=pd.read_csv("Data_Pro/"+str(i)+".csv")
            df=pd.DataFrame(df)
            merge=pd.read_csv("Data_Pro/"+merge_file)
            merge=pd.DataFrame(merge)
            pd.merge(merge,df,on=on,how=how).to_csv("Data_Pro/"+merge_file)
...
def main():
    ...
    col_names_english = ["air_temp", "air_humidity", "land_temp", "land_humidity", "land_conduct", "light","wind_sp","wind_direct", \
                         "land_PH", "neg_oxy_ion_concent", "rainy_num", "ultraviolet_radiation", "atm_pres","photosynthetic_rad", \
                         "sunshine_hours", "air_CO2", "salt", "water_dissolved_oxy", "water_dissolved_CO2","land_temp-5cm","land_hum-5cm", \
    ...
    # dp.merge_csv_by_MeasureTime("merge.csv",list_csv)
    ...
    dp.process_time()
    # print(pd.read_csv("Data_Pro/1.csv"))


    # data=pd.read_csv("Data_Pro/merge.csv")
    # print(data)
